...
const AnimatedLogin = () => {
  ...
  // Define card class based on theme
  ...
  const subtextClass = theme === 'dark' ? 'text-gray-200' : 'text-gray-700';
  const inputClass = theme === 'dark' 
    ? 'bg-white/5 border border-white/10 text-white placeholder:text-gray-400' 
    : 'bg-white/70 border border-gray-300/50 text-gray-900 placeholder:text-gray-500';

  return (
    <div className={`relative min-h-screen w-full overflow-hidden flex items-center justify-center ${bgClass}`}>
      {/* Spline 3D background */}
      <div className="absolute inset-0 w-full h-full -z-10">
        {!splineLoaded && (
          <div className="absolute inset-0 flex items-center justify-center">
            <LucideLoader2 className={`w-12 h-12 ${textClass} animate-spin`} />
          </div>
        )}
        <Spline 
        ...
        >
          <Card className={cardBgClass}>
            <CardHeader className="space-y-1">
              ...
              >
                <CardTitle className={`text-2xl font-bold text-center ${textClass}`}>Welcome Back</CardTitle>
                <CardDescription className={`text-center ${subtextClass}`}>
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  >
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className={subtextClass}>Email</FormLabel>
                          <FormControl>
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <div className="flex justify-between">
                            <FormLabel className={subtextClass}>Password</FormLabel>
                            <Link to="/auth/reset-password" className={`text-xs ${theme === 'dark' ? 'text-blue-300 hover:text-blue-200' : 'text-blue-600 hover:text-blue-700'} transition-colors`}>
                              Forgot password?
                            </Link>
                          </div>
                          <FormControl>
            ...
            <CardFooter className="flex justify-center">
              ...
              >
                ...
                <Link to="/auth/signup" className={`${theme === 'dark' ? 'text-blue-300 hover:text-blue-200' : 'text-blue-600 hover:text-blue-700'} transition-colors font-medium`}>
                  Sign up
                </Link>
              </motion.p>
            </CardFooter>
          ...
          >
            <Link to="/" className={`text-sm ${theme === 'dark' ? 'text-gray-300 hover:text-white' : 'text-gray-600 hover:text-gray-900'} transition-colors`}>
              Return to Home
            </Link>
          </motion.div>
        </motion.div>
